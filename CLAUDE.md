...
- **Les fichiers `.well-known` doivent répondre 200, `application/json`, sans redirection.** `trailingSlash: true` redirige toute adresse sans extension — mais pas celles dont un segment contient un point, donc pas `/.well-known/`.
- **Vercel ignore un `content-type` déclaré dans `headers`** pour un fichier statique : il le déduit de l'extension (le `cache-control` de la même règle, lui, s'applique). Le fichier Apple est donc **stocké en `.json`** et servi sans extension par une **réécriture** ; le fichier sans extension ne doit pas exister, sinon le système de fichiers reprend la main avec le mauvais type.
- **Une réécriture doit exister dans les deux formes**, avec et sans barre oblique finale : `/video/abc` est redirigé en 308 vers `/video/abc/` avant d'atteindre les réécritures. Une seule forme déclarée = 404 en production alors que tout marche en local.
